...
for(p in levels(periodz)){
  ...
  utility_zero=subset(utility_state0_MA_all, period==p, select=c(tripid, cost, tot_keep_scup, tot_rel_scup,
 ...
 for(d in 1:1) {
  ls(trip_data)
    ...
    v0 =  subset(utility_state0_MA_all, period==p, select=c(tripid, alt, v0_col_sum))
...
pds_new_all_MA_alt=list.stack(pds_new, fill=TRUE)
pds_new_all_MA_alt[is.na(pds_new_all_MA_alt)] = 0
pds_new_all_MA_alt$state = state1
