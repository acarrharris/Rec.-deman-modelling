...
###New Jersey boat
catch_data <- read_excel("observed_catch_NJ_19_bt.xlsx")
...
write_xlsx(catch_data_sim, "NJ_catch_data_sim1_bt.xlsx") 



###New Jersey boat
catch_data <- read_excel("observed_catch_NJ_19_sh.xlsx")
#catch_data <- readRDS("observed_catch_NJ_19.rds")
...
write_xlsx(catch_data_sim, "NJ_catch_data_sim1_sh.xlsx") 
...
directed_trips = data.frame(read_excel("directed_trips_NJ_by_mode.xlsx"))                                                                            
directed_trips$dtrip=round(directed_trips$dtrip_2019)
directed_trips= subset(directed_trips, state == state1 & mode=="sh")
...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = data.frame(read_excel("NJ_catch_data_sim1_sh.xlsx"))                                                                            
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=.962, 1,0) 
...
# ((964810-sum(pds_new_all_NJ$tot_keep))/964810)*100
# ((9382985-sum(pds_new_all_NJ$tot_rel))/9382985)*100
...
sum(pds_new_all_NJ$tot_keep)
((143345-sum(pds_new_all_NJ$tot_keep))/143345)*100
((3685185-sum(pds_new_all_NJ$tot_rel))/3685185)*100
