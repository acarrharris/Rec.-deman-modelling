...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=.91, 1,0) 
    ...
    if(scup_bag>0){
      ...
      scup_catch_data1$keep = ifelse(scup_catch_data1$uniform>=0.75, 1,0) 
    ...
    # merge catch data with other trip data 
...
for(d in 1:100) {
  ...
  utilites_NY[[d]]= param_draws_NY
  
}

utilites_NY_all=list.stack(utilites_NY, fill=TRUE)
...
for(p in levels(periodz)){
  ...
  costs_new_NY[[p]] = subset(trip_data, select=c(tripid, cost, catch_draw, tot_keep, tot_rel, tot_sf_catch,
  ...
  names(costs_new_NY[[p]])[names(costs_new_NY[[p]]) == "tot_keep_scup"] = "tot_keep_scup_base"
  names(costs_new_NY[[p]])[names(costs_new_NY[[p]]) == "tot_rel_scup"] = "tot_rel_scup_base"
  
  
  costs_new_NY[[p]]$period = p
  ...
  for(d in 1:1) {
    ...
    parameter_draws_NY[[d]]=aggregate_trip_data
    
  }
  
  # Combine the output from all the utility parameter draws (for now we only have one)
  parameter_draws_all_NY = as.data.frame(bind_rows(parameter_draws_NY[[1]]))
  parameter_draws_all_NY[is.na(parameter_draws_all_NY)] = 0
  rownames(parameter_draws_all_NY) = NULL
  
  
  parameter_draws_all_NY$period=p
  pds_new[[p]]=parameter_draws_all_NY
...
pds_new_all_NY$alt_regs = 0
pds_new_all_NY= subset(pds_new_all_NY, select=-c(Group.1, tot_sf_catch, tot_bsb_catch))
rm(pds_new)
...
costs_new_all_NY=list.stack(costs_new_NY, fill=TRUE)
costs_new_all_NY[is.na(costs_new_all_NY)] = 0
rm(costs_new_NY)
...
sum(pds_new_all_NY$tot_keep)
((1186402-sum(pds_new_all_NY$tot_keep))/1186402)*100

sum(pds_new_all_NY$tot_rel)
((12345146-sum(pds_new_all_NY$tot_rel))/12345146)*100

pds_new_all_NY$tot_sf_cat=pds_new_all_NY$tot_keep+pds_new_all_NY$tot_rel
sum(pds_new_all_NY$tot_sf_cat)
(((1186402+12345146)-sum(pds_new_all_NY$tot_sf_cat))/(1186402+12345146))*100




sum(pds_new_all_NY$tot_keep_bsb)
((757305.3-sum(pds_new_all_NY$tot_keep_bsb))/757305.3)*100

sum(pds_new_all_NY$tot_rel_bsb)
((5430704-sum(pds_new_all_NY$tot_rel_bsb))/5430704)*100

pds_new_all_NY$tot_bsb_cat=pds_new_all_NY$tot_keep_bsb+pds_new_all_NY$tot_rel_bsb
sum(pds_new_all_NY$tot_bsb_cat)
(((757305.3+5430704)-sum(pds_new_all_NY$tot_bsb_cat))/(757305.3+5430704))*100




sum(pds_new_all_NY$tot_keep_scup)
((1641323-sum(pds_new_all_NY$tot_keep_scup))/1641323)*100

sum(pds_new_all_NY$tot_rel_scup)
((5648786 -sum(pds_new_all_NY$tot_rel_scup))/5648786 )*100

pds_new_all_NY$tot_scup_cat=pds_new_all_NY$tot_keep_scup+pds_new_all_NY$tot_rel_scup
sum(pds_new_all_NY$tot_scup_cat)
(((1641323+5648786)-sum(pds_new_all_NY$tot_scup_cat))/(1641323+5648786))*100
