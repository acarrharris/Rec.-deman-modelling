...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    #   ###
    catch_size_data$post_bag_fish=ifelse(catch_size_data$csum_keep>fluke_bag, 1,0)
    catch_size_data= subset(catch_size_data,post_bag_fish==0 )
    catch_size_data <- subset(catch_size_data, select=-c(post_bag_fish ))
    #   ###
