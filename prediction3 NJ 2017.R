...
#           selectivity and projected population abundances at length. xlsx
...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(read_excel("predicted_catch_NJ.xlsx"))                                                                            
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    param_draws_NJ_prediction <- subset(param_draws_NJ, parameter_draw==d)
    trip_data <-  merge(param_draws_NJ_prediction,trip_data,by="tripid")
    
    
    # Expected utility (prediction year)
    trip_data$vA <- 
      trip_data$beta_sqrt_sf_keep*sqrt(trip_data$tot_keep) +
