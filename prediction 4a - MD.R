...
for(p in levels(periodz)){
  ...
  charter=round(max_trip*.02)
  headboat=round(max_trip*.05)
  private=round(max_trip*.33)
  ...
  for(d in 1:1) {
    ...
    utility_state0_MD[[p]]=mean_trip_data_s0
...
utility_state0_MD_all=list.stack(utility_state0_MD, fill=TRUE)
