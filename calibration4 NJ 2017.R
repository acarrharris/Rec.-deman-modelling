...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = data.frame(read_excel("NJ_catch_data_sim1_2017.xlsx"))                                                                            
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=0.84, 1,0) 
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.78, 1,0) 
    ...
    if(scup_bag>0){
      ...
      scup_catch_data1$keep = ifelse(scup_catch_data1$uniform>=.25, 1,0) 
    ...
    if(wf_bag>0){
      ...
      wf_catch_data1$keep = ifelse(wf_catch_data1$uniform>=.2, 1,0) 
...
for(d in 1:100) {
  ...
  utilites_NJ[[d]]= param_draws_NJ
  
}

utilites_NJ_all=list.stack(utilites_NJ, fill=TRUE)
...
((1211186-sum(pds_new_all_NJ$tot_keep))/1211186)*100

sum(pds_new_all_NJ$tot_rel)
((7784823-sum(pds_new_all_NJ$tot_rel))/7784823)*100

pds_new_all_NJ$tot_sf_cat=pds_new_all_NJ$tot_keep+pds_new_all_NJ$tot_rel
sum(pds_new_all_NJ$tot_sf_cat)
(((1211186+7784823)-sum(pds_new_all_NJ$tot_sf_cat))/(1211186+7784823))*100



sum(pds_new_all_NJ$tot_keep_bsb)
((482341.1-sum(pds_new_all_NJ$tot_keep_bsb))/482341.1)*100

sum(pds_new_all_NJ$tot_rel_bsb)
((3642644-sum(pds_new_all_NJ$tot_rel_bsb))/3642644)*100

pds_new_all_NJ$tot_bsb_cat=pds_new_all_NJ$tot_keep_bsb+pds_new_all_NJ$tot_rel_bsb
sum(pds_new_all_NJ$tot_bsb_cat)
(((482341.1+3642644)-sum(pds_new_all_NJ$tot_bsb_cat))/(482341.1+3642644))*100


sum(pds_new_all_NJ$tot_keep_scup)
((295260.7-sum(pds_new_all_NJ$tot_keep_scup))/295260.7)*100

sum(pds_new_all_NJ$tot_rel_scup)
((562624.5-sum(pds_new_all_NJ$tot_rel_scup))/562624.5)*100

pds_new_all_NJ$tot_scup_cat=pds_new_all_NJ$tot_keep_scup+pds_new_all_NJ$tot_rel_scup
(((295260.7+562624.5)-sum(pds_new_all_NJ$tot_scup_cat))/(295260.7+562624.5))*100


sum(pds_new_all_NJ$tot_keep_wf)
((23756.89-sum(pds_new_all_NJ$tot_keep_wf))/23756.89)*100

sum(pds_new_all_NJ$tot_rel_wf)
((119986.3 -sum(pds_new_all_NJ$tot_rel_wf))/119986.3 )*100

pds_new_all_NJ$tot_wf_cat=pds_new_all_NJ$tot_keep_wf+pds_new_all_NJ$tot_rel_wf
(((23756.89+119986.3)-sum(pds_new_all_NJ$tot_wf_cat))/(23756.89+119986.3))*100
