...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=0.89, 1,0) 
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.94, 1,0)
    ...
    if(wf_bag>0){
      ...
      wf_catch_data1$keep = ifelse(wf_catch_data1$uniform>=.96, 1,0) 
...
((98061.17-sum(pds_new_all_DE$tot_keep))/98061.17)*100

sum(pds_new_all_DE$tot_rel)
((591363.7-sum(pds_new_all_DE$tot_rel))/591363.7)*100

pds_new_all_DE$tot_sf_cat=pds_new_all_DE$tot_keep+pds_new_all_DE$tot_rel
sum(pds_new_all_DE$tot_sf_cat)
(((98061.17+591363.7)-sum(pds_new_all_DE$tot_sf_cat))/(98061.17+591363.7))*100



sum(pds_new_all_DE$tot_keep_bsb)
((21764.8-sum(pds_new_all_DE$tot_keep_bsb))/21764.8)*100

sum(pds_new_all_DE$tot_rel_bsb)
((209785.2-sum(pds_new_all_DE$tot_rel_bsb))/209785.2)*100

pds_new_all_DE$tot_bsb_cat=pds_new_all_DE$tot_keep_bsb+pds_new_all_DE$tot_rel_bsb
sum(pds_new_all_DE$tot_bsb_cat)
(((21764.8+209785.2)-sum(pds_new_all_DE$tot_bsb_cat))/(21764.8+209785.2))*100




sum(pds_new_all_DE$tot_keep_wf)
((1051.58-sum(pds_new_all_DE$tot_keep_wf))/1051.58)*100

sum(pds_new_all_DE$tot_rel_wf)
((13885.54 -sum(pds_new_all_DE$tot_rel_wf))/13885.54 )*100

pds_new_all_DE$tot_wf_cat=pds_new_all_DE$tot_keep_wf+pds_new_all_DE$tot_rel_wf
sum(pds_new_all_DE$tot_wf_cat)
(((1051.58+13885.54)-sum(pds_new_all_DE$tot_wf_cat))/(1051.58+13885.54))*100
