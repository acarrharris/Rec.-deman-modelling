...
selectivity = data.frame(read_excel("rec_selectivity_coastwide_cdf_star_raw.xlsx"))
...
numbers_at_length_new <-subset(numbers_at_length_new, N_l!=0)
...
numbers_at_length <-subset(numbers_at_length_new, select=c(l_in_bin, C_l_new))
numbers_at_length$C_l_new=round(numbers_at_length$C_l_new)

tot_cat_predicted=sum(numbers_at_length$C_l_new)
tot_cat_base=sum(selectivity$C_l)

#Create a factor that expands total catch in the prediction year
catch_expansion_factor=tot_cat_predicted/tot_cat_base
...
source("predicted_catch_per_trip - assessment CAL.R")
...
# north region
row_inds = seq_len(nrow(numbers_at_length))
numbers_at_length_NO = numbers_at_length[c(rep(row_inds, numbers_at_length$C_l_new)), ]
...
#New Jersey
row_inds = seq_len(nrow(numbers_at_length))
numbers_at_length_NJ = numbers_at_length[c(rep(row_inds, numbers_at_length$C_l_new)), ]
...
#Southern region
row_inds = seq_len(nrow(numbers_at_length))
numbers_at_length_SO = numbers_at_length[c(rep(row_inds, numbers_at_length$C_l_new)), ]
...
fitted_sizes_region_all_y2 = as.data.frame(bind_rows(numbers_at_length_SO, numbers_at_length_NJ, numbers_at_length_NO))
