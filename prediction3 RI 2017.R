...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(read_excel("predicted_catch_RI.xlsx"))                                                                            
    ...
    bsb_sc_data <- subset(costs_new_all_RI, period==p & catch_draw==i, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>%
      tibble()
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    param_draws_RI_prediction <- subset(param_draws_RI, parameter_draw==d)
...
pds_new_all_RI=subset(pds_new_all_RI, select=-c(Group.1, tot_keep_sf_base, tot_rel_sf_base, 
