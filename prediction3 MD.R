...
#           selectivity and projected population abundances at length. lsx
...
for(p in levels(periodz)){
  ...
  cost_data = subset(costs_new_all_MD, period == p, select=-c(period, tot_sf_catch))
  ...
  for(d in 1:1) {
    ...
    param_draws_MD_prediction = subset(param_draws_MD, parameter_draw==d)
    trip_data =  merge(param_draws_MD_prediction,trip_data,by="tripid")
...
pds_new_all_MD$alt_regs = 1
pds_new_all_MD=subset(pds_new_all_MD, select=-c(Group.1, tot_keep_sf_base, tot_rel_sf_base, 
