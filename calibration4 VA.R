...
for(p in levels(periodz)){
  ...
  headboat=round(max_trip*.005)
  private=round(max_trip*.45)
  ...
  for(d in 1:1) {
    ...
    param_draws_VA = as.data.frame(1:10000)
    names(param_draws_VA)[names(param_draws_VA) == "1:10000"] = "tripid"
    
    param_draws_VA$beta_sqrt_sf_keep = rnorm(10000, mean = 0.521, sd = 0.464)
    param_draws_VA$beta_sqrt_sf_release = rnorm(10000, mean = 0.108, sd = 0.221)
    param_draws_VA$beta_sqrt_bsb_keep = rnorm(10000, mean = 0.192, sd = 0.200)
    param_draws_VA$beta_sqrt_bsb_release = rnorm(10000, mean = 0, sd = 0.131)
    param_draws_VA$beta_sqrt_wf_keep = rnorm(10000, mean = 0.231, sd =  0.393)
    param_draws_VA$beta_sqrt_wf_release = rnorm(10000, mean = 0, sd = 0.146)
    param_draws_VA$beta_sqrt_rd_keep = rnorm(10000, mean = 0.454, sd =  0.601)
    param_draws_VA$beta_sqrt_rd_release = rnorm(10000, mean = 0.081, sd = 0.356)
    param_draws_VA$beta_opt_out = rnorm(10000, mean = -3.908, sd = 2.918)
    param_draws_VA$beta_striper_blue = rnorm(10000, mean = 0.454, sd = 1.991)
    param_draws_VA$beta_cost = rnorm(10000, mean = -0.008, sd = 0)
...
((0-sum(pds_new_all_VA$tot_keep_rd))/0)*100

sum(pds_new_all_VA$tot_rel_rd)
((395773.06-sum(pds_new_all_VA$tot_rel_rd))/395773.06)*100

pds_new_all_VA$tot_rd_cat=pds_new_all_VA$tot_keep_rd+pds_new_all_VA$tot_rel_rd
sum(pds_new_all_VA$tot_rd_cat)
((395773.06-sum(pds_new_all_VA$tot_rd_cat))/395773.06)*100
