...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(read_excel("predicted_catch_MA.xlsx"))                                                                            
    ...
    bsb_sc_data<-subset(costs_new_all_MA, period==p & catch_draw==i, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>%
      tibble()
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    param_draws_MA_prediction <- subset(param_draws_MA, parameter_draw==d)
