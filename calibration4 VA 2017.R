...
directed_trips = data.frame(read_excel("directed_trips_regions_bimonthly_2017.xlsx")) 
directed_trips$dtrip=round(directed_trips$dtrip_2017)
...
for(p in levels(periodz)){
  ...
  n_trips = mean(directed_trips_p$dtrip_2017)
  #n_draws = min(1000,n_trips*2.5 )
  n_draws = n_drawz
  
  fluke_bag = mean(directed_trips_p$fluke_bag_2017)
  fluke_min = mean(directed_trips_p$fluke_min_2017)
  bsb_bag = mean(directed_trips_p$bsb_bag_2017)
  bsb_min = mean(directed_trips_p$bsb_min_2017)
  wf_bag = mean(directed_trips_p$wf_bag_2017)
  wf_min = mean(directed_trips_p$wf_min_2017) 
  rd_bag = mean(directed_trips_p$rd_bag_2017)
  rd_min = mean(directed_trips_p$rd_min_2017) 
  rd_max = mean(directed_trips_p$rd_max_2017) 
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = data.frame(read_excel("SO_catch_data_sim1_2017.xlsx"))                                                                            
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=0.88, 1,0) 
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.975, 1,0) 
    ...
    #import wf data
    wf_catch_data = data.frame(read_excel("other_species_fitted_catch_2017.xlsx"))                                                                            
    ...
    if(wf_bag>0){
      ...
      wf_catch_data1$keep = ifelse(wf_catch_data1$uniform>=.99, 1,0)
    ...
    #import rd data
    rd_catch_data = data.frame(read_excel("other_species_fitted_catch_2017.xlsx"))                                                                            
    ...
    if(rd_bag>0){
      ...
      rd_catch_data1$keep = ifelse(rd_catch_data1$uniform>=.2, 1,0) 
...
for(d in 1:100) {
  ...
  param_draws_VA$parameter_draw=d
  param_draws_VA <- param_draws_VA[1:n_drawz, ] 
  
  
  utilites_VA[[d]]= param_draws_VA
  
}

utilites_VA_all=list.stack(utilites_VA, fill=TRUE)
...
for(p in levels(periodz)){
  ...
  costs_new_VA[[p]] = subset(trip_data, select=c(tripid, cost, catch_draw, tot_keep, tot_rel, tot_sf_catch,
                                                 tot_keep_bsb,tot_rel_bsb, tot_keep_wf, tot_rel_wf, tot_keep_rd, tot_rel_rd  ))
  
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_keep"] = "tot_keep_sf_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_rel"] = "tot_rel_sf_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_keep_bsb"] = "tot_keep_bsb_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_rel_bsb"] = "tot_rel_bsb_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_keep_wf"] = "tot_keep_wf_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_rel_wf"] = "tot_rel_wf_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_keep_rd"] = "tot_keep_rd_base"
  names(costs_new_VA[[p]])[names(costs_new_VA[[p]]) == "tot_rel_rd"] = "tot_rel_rd_base"
  
  costs_new_VA[[p]]$period = p
  
  
  
  
  #set up an output file for each draw of utility parameters
  parameter_draws_VA = list()
  
  for(d in 1:1) {
    
    param_draws_VA= utilites_VA[[1]]
    ...
    parameter_draws_VA[[d]]=aggregate_trip_data
    
  }
  
  # Combine the output from all the utility parameter draws (for now we only have one)
  parameter_draws_all_VA = as.data.frame(bind_rows(parameter_draws_VA[[1]]))
  parameter_draws_all_VA[is.na(parameter_draws_all_VA)] = 0
  rownames(parameter_draws_all_VA) = NULL
  
  
  parameter_draws_all_VA$period=p
  pds_new[[p]]=parameter_draws_all_VA
...
pds_new_all_VA$alt_regs = 0
pds_new_all_VA= subset(pds_new_all_VA, select=-c(Group.1, tot_sf_catch, tot_bsb_catch))
rm(pds_new)
...
costs_new_all_VA=list.stack(costs_new_VA, fill=TRUE)
costs_new_all_VA[is.na(costs_new_all_VA)] = 0
rm(costs_new_VA)
...
sum(pds_new_all_VA$tot_keep)
((188346.1-sum(pds_new_all_VA$tot_keep))/188346.1)*100

sum(pds_new_all_VA$tot_rel)
((1610215-sum(pds_new_all_VA$tot_rel))/1610215)*100

pds_new_all_VA$tot_sf_cat=pds_new_all_VA$tot_keep+pds_new_all_VA$tot_rel
sum(pds_new_all_VA$tot_sf_cat)
(((188346.1+1610215)-sum(pds_new_all_VA$tot_sf_cat))/(188346.1+1610215))*100




sum(pds_new_all_VA$tot_keep_bsb)
((14358.78-sum(pds_new_all_VA$tot_keep_bsb))/14358.78)*100

sum(pds_new_all_VA$tot_rel_bsb)
((861111.2-sum(pds_new_all_VA$tot_rel_bsb))/861111.2)*100

pds_new_all_VA$tot_bsb_cat=pds_new_all_VA$tot_keep_bsb+pds_new_all_VA$tot_rel_bsb
sum(pds_new_all_VA$tot_bsb_cat)
(((14358.78+861111.2)-sum(pds_new_all_VA$tot_bsb_cat))/(14358.78+861111.2))*100




sum(pds_new_all_VA$tot_keep_wf)
((259.8-sum(pds_new_all_VA$tot_keep_wf))/259.8)*100

sum(pds_new_all_VA$tot_rel_wf)
((82353.68-sum(pds_new_all_VA$tot_rel_wf))/82353.68)*100

pds_new_all_VA$tot_wf_cat=pds_new_all_VA$tot_keep_wf+pds_new_all_VA$tot_rel_wf
sum(pds_new_all_VA$tot_wf_cat)
(((259.8+82353.68)-sum(pds_new_all_VA$tot_wf_cat))/(259.8+82353.68))*100





sum(pds_new_all_VA$tot_keep_rd)
((180137.3-sum(pds_new_all_VA$tot_keep_rd))/180137.3)*100

sum(pds_new_all_VA$tot_rel_rd)
((89872.01-sum(pds_new_all_VA$tot_rel_rd))/89872.01)*100

pds_new_all_VA$tot_rd_cat=pds_new_all_VA$tot_keep_rd+pds_new_all_VA$tot_rel_rd
sum(pds_new_all_VA$tot_rd_cat)
(((180137.3+89872.01)-sum(pds_new_all_VA$tot_rd_cat))/(180137.3+89872.01))*100
