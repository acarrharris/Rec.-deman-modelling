...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sc_data=subset(costs_new_all_RI, period ==p & catch_draw==i, select=c(tripid,tot_keep_scup_base, tot_rel_scup_base )) 
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    utility_state0_RI[[p]]=mean_trip_data_s0
...
utility_state0_RI_all=list.stack(utility_state0_RI, fill=TRUE)
