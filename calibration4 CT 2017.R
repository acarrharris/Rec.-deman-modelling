...
for(p in levels(periodz)){
  ...
  scup_bag = mean(directed_trips_p$scup_bag_2017)
  scup_min = mean(directed_trips_p$scup_min_2017)
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = data.frame(read_excel("NO_catch_data_sim1_2017.xlsx"))                                                                            
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=.9, 1,0) 
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.63, 1,0) 
    ...
    #import scup data
    scup_catch_data = data.frame(read_excel("other_species_fitted_catch_2017.xlsx"))                                                                            
    ...
    if(scup_bag>0){
      ...
      scup_catch_data1$keep = ifelse(scup_catch_data1$uniform>=.4, 1,0) 
...
for(d in 1:100) {
  ...
  param_draws_CT$parameter_draw=d
  param_draws_CT <- param_draws_CT[1:n_drawz, ] 
  
  
  utilites_CT[[d]]= param_draws_CT
  
}

utilites_CT_all=list.stack(utilites_CT, fill=TRUE)
...
for(p in levels(periodz)){
  ...
  costs_new_CT[[p]] = subset(trip_data, select=c(tripid, cost, catch_draw, tot_keep, tot_rel, tot_sf_catch,
                                                 tot_keep_bsb,tot_rel_bsb,tot_keep_scup, tot_rel_scup  ))
  
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_keep"] = "tot_keep_sf_base"
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_rel"] = "tot_rel_sf_base"
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_keep_bsb"] = "tot_keep_bsb_base"
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_rel_bsb"] = "tot_rel_bsb_base"
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_keep_scup"] = "tot_keep_scup_base"
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_rel_scup"] = "tot_rel_scup_base"
  
  
  costs_new_CT[[p]]$period = p
  ...
  for(d in 1:1) {
    ...
    param_draws_CT= utilites_CT[[1]]
    trip_data =  merge(param_draws_CT,trip_data,by="tripid")   
  ...
  parameter_draws_all_CT = as.data.frame(bind_rows(parameter_draws_CT[[1]]))
  parameter_draws_all_CT[is.na(parameter_draws_all_CT)] = 0
  rownames(parameter_draws_all_CT) = NULL
  
  
  parameter_draws_all_CT$period=p
  pds_new[[p]]=parameter_draws_all_CT
  
}


pds_new_all_CT=list.stack(pds_new, fill=TRUE)

pds_new_all_CT[is.na(pds_new_all_CT)] = 0
pds_new_all_CT$state = state1
pds_new_all_CT$alt_regs = 0
pds_new_all_CT= subset(pds_new_all_CT, select=-c(Group.1, tot_sf_catch, tot_bsb_catch))
rm(pds_new)

# write_xlsx(pds_new_all_MA,"MA_calibration_output_check.xlsx")
...
costs_new_all_CT=list.stack(costs_new_CT, fill=TRUE)
costs_new_all_CT[is.na(costs_new_all_CT)] = 0
rm(costs_new_CT)




sum(pds_new_all_CT$tot_keep)
((121086.8-sum(pds_new_all_CT$tot_keep))/121086.8)*100

sum(pds_new_all_CT$tot_rel)
((811258.4 -sum(pds_new_all_CT$tot_rel))/811258.4 )*100

pds_new_all_CT$tot_sf_cat=pds_new_all_CT$tot_keep+pds_new_all_CT$tot_rel
sum(pds_new_all_CT$tot_sf_cat)
(((121086.8+811258.4 )-sum(pds_new_all_CT$tot_sf_cat))/(121086.8+811258.4 ))*100




sum(pds_new_all_CT$tot_keep_bsb)
((179547.8-sum(pds_new_all_CT$tot_keep_bsb))/179547.8)*100

sum(pds_new_all_CT$tot_rel_bsb)
((847338.3-sum(pds_new_all_CT$tot_rel_bsb))/847338.3)*100

pds_new_all_CT$tot_bsb_cat=pds_new_all_CT$tot_keep_bsb+pds_new_all_CT$tot_rel_bsb
sum(pds_new_all_CT$tot_bsb_cat)
(((179547.8+847338.3)-sum(pds_new_all_CT$tot_bsb_cat))/(179547.8+847338.3))*100



sum(pds_new_all_CT$tot_keep_scup)
((356977.6-sum(pds_new_all_CT$tot_keep_scup))/356977.6)*100

sum(pds_new_all_CT$tot_rel_scup)
(( 815109.4-sum(pds_new_all_CT$tot_rel_scup))/ 815109.4)*100

pds_new_all_CT$tot_scup_cat=pds_new_all_CT$tot_keep_scup+pds_new_all_CT$tot_rel_scup
sum(pds_new_all_CT$tot_scup_cat)
(((356977.6+815109.4)-sum(pds_new_all_CT$tot_scup_cat))/(356977.6+815109.4))*100
