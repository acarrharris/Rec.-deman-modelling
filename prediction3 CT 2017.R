...
calibration_data  <- data.frame(read_excel("calibration_output_by_period_2017.xlsx"))
...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(read_excel("predicted_catch_CT.xlsx"))                                                                            
    ...
    bsb_sc_data <- subset(costs_new_all_CT, period==p & catch_draw==i, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>%
      tibble()
    ...
    bsb_sc_data <- subset(bsb_sc_data, period==p)
    trip_data <- merge(trip_data,bsb_sc_data,by="tripid")
    trip_data[is.na(trip_data)] <- 0        
    
    #end Gavin code insert
...
for(p in levels(periodz)){
  ...
  cost_data <- subset(costs_new_all_CT, period == p, select=-c(period, tot_sf_catch))
  ...
  for(d in 1:1) {
    ...
    param_draws_CT_prediction <- subset(param_draws_CT, parameter_draw==d)
    trip_data <-  merge(param_draws_CT_prediction,trip_data,by="tripid")
    ...
    aggregate_trip_data <- subset(aggregate_trip_data, select=-c(Group.1, Group.1, tripid, catch_draw, period, cost, vA , v0, sim, prob0, parameter_draw))
...
pds_new_all_CT$alt_regs = 1
pds_new_all_CT=subset(pds_new_all_CT, select=-c(Group.1, tot_keep_sf_base, tot_rel_sf_base, 
