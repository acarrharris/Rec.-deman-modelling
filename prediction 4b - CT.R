...
#for(p in levels(periodz)){
  ...
  #for(d in 1:1){
    ...
    v0 =  subset(utility_state0_CT_all, period==p, select=c(tripid, alt, v0_col_sum))
...
pds_new_all_CT_alt=list.stack(pds_new, fill=TRUE)
pds_new_all_CT_alt[is.na(pds_new_all_CT_alt)] = 0
pds_new_all_CT_alt$state = state1
