...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.945, 1,0) 
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    utility_state0_DE[[p]]=mean_trip_data_s0
...
utility_state0_DE_all=list.stack(utility_state0_DE, fill=TRUE)
