...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=0.8, 1,0) 
    ...
    if(scup_bag>0){
      ...
      scup_catch_data1$keep = ifelse(scup_catch_data1$uniform>=.70, 1,0) 
...
for(d in 1:100) {
  ...
  utilites_RI[[d]]= param_draws_RI
  
}

utilites_RI_all=list.stack(utilites_RI, fill=TRUE)
...
((155885.7-sum(pds_new_all_RI$tot_keep))/155885.7)*100

sum(pds_new_all_RI$tot_rel)
((783603.9-sum(pds_new_all_RI$tot_rel))/783603.9)*100

pds_new_all_RI$tot_sf_cat=pds_new_all_RI$tot_keep+pds_new_all_RI$tot_rel
sum(pds_new_all_RI$tot_sf_cat)
(((155885.7+783603.9)-sum(pds_new_all_RI$tot_sf_cat))/(155885.7+783603.9))*100




sum(pds_new_all_RI$tot_keep_bsb)
((146709.9-sum(pds_new_all_RI$tot_keep_bsb))/146709.9)*100

sum(pds_new_all_RI$tot_rel_bsb)
((449731.3-sum(pds_new_all_RI$tot_rel_bsb))/449731.3)*100

pds_new_all_RI$tot_bsb_cat=pds_new_all_RI$tot_keep_bsb+pds_new_all_RI$tot_rel_bsb
sum(pds_new_all_RI$tot_bsb_cat)
(((146709.9+449731.3)-sum(pds_new_all_RI$tot_bsb_cat))/(146709.9+449731.3))*100
...
((104834.7-sum(pds_new_all_RI$tot_keep_scup))/104834.7)*100

sum(pds_new_all_RI$tot_rel_scup)
((146985.4-sum(pds_new_all_RI$tot_rel_scup))/146985.4)*100

pds_new_all_RI$tot_scup_cat=pds_new_all_RI$tot_keep_scup+pds_new_all_RI$tot_rel_scup
sum(pds_new_all_RI$tot_scup_cat)
(((104834.7+146985.4)-sum(pds_new_all_RI$tot_scup_cat))/(104834.7+146985.4))*100