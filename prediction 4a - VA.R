...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    size_data_sf = data.frame(read_excel("rec_selectivity_MRIP_ALS.xlsx")) ###
    size_data_sf = subset(size_data_sf, region==region1, select= c(C_l, l_in_bin)) ###
    
    size_data_sf$fitted_prob = size_data_sf$C_l/sum(size_data_sf$C_l) ###
    size_data_sf$fitted_length= size_data_sf$l_in_bin ###
    size_data_sf = subset(size_data_sf, select= c(fitted_length, fitted_prob)) ###
    ...
    wf_rd_data=subset(costs_new_all_VA, period ==p & catch_draw==i, select=c(tripid, tot_keep_wf_base, tot_rel_wf_base, tot_keep_rd_base, tot_rel_rd_base )) 
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    mean_trip_data_s0=subset(mean_trip_data, select=c(tripid, period, alt, prob0, v0_col_sum, 
                                                      cost,beta_sqrt_rd_keep, beta_sqrt_rd_release,
    ...
    utility_state0_VA[[p]]=mean_trip_data_s0
    ...
    mean_trip_data = subset(mean_trip_data, alt==1, select=-c(alt, opt_out, striper_blue, v0_optout, v0_striper_blue, v0_row_sum, v0_col_sum,
                                                              ...
                                                              beta_sqrt_rd_release, beta_sqrt_rd_keep))
...
utility_state0_VA_all=list.stack(utility_state0_VA, fill=TRUE)
