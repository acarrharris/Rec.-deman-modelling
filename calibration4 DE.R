...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=0.875, 1,0) 
    ...
    if(wf_bag>0){
      ...
      wf_catch_data1$keep = ifelse(wf_catch_data1$uniform>=0.87, 1,0) 
...
for(p in levels(periodz)){
  ...
  costs_new_DE[[p]] = subset(trip_data, select=c(tripid, cost, catch_draw, tot_keep, tot_rel, tot_sf_catch,
  ...
  names(costs_new_DE[[p]])[names(costs_new_DE[[p]]) == "tot_keep_wf"] = "tot_keep_wf_base"
  names(costs_new_DE[[p]])[names(costs_new_DE[[p]]) == "tot_rel_wf"] = "tot_rel_wf_base"
  ...
  for(d in 1:1) {
    ...
    param_draws_DE = as.data.frame(1:10000)
    names(param_draws_DE)[names(param_draws_DE) == "1:10000"] = "tripid"
    
    param_draws_DE$beta_sqrt_sf_keep = rnorm(10000, mean = 0.807, sd = 0.599)
    param_draws_DE$beta_sqrt_sf_release = rnorm(10000, mean = 0, sd = 0.317)
    param_draws_DE$beta_sqrt_bsb_keep = rnorm(10000, mean = 0.239, sd = 0.287)
    param_draws_DE$beta_sqrt_bsb_release = rnorm(10000, mean = 0, sd = 0.160)
    param_draws_DE$beta_sqrt_wf_keep = rnorm(10000, mean = 0.379, sd =  0.381)
    param_draws_DE$beta_sqrt_wf_release = rnorm(10000, mean = 0.064, sd = 0.227)
    param_draws_DE$beta_opt_out = rnorm(10000, mean = -2.963, sd = 2.448)
    param_draws_DE$beta_striper_blue = rnorm(10000, mean = 0.645, sd = 1.900)
    param_draws_DE$beta_cost = rnorm(10000, mean = -0.009, sd = 0)
    ...
    parameter_draws_DE[[d]]=aggregate_trip_data
    
  }
  
  # Combine the output from all the utility parameter draws (for now we only have one)
  parameter_draws_all_DE = as.data.frame(bind_rows(parameter_draws_DE[[1]]))
  parameter_draws_all_DE[is.na(parameter_draws_all_DE)] = 0
  rownames(parameter_draws_all_DE) = NULL
  
  
  parameter_draws_all_DE$period=p
  pds_new[[p]]=parameter_draws_all_DE
...
sum(pds_new_all_DE$tot_keep)
((91025-sum(pds_new_all_DE$tot_keep))/91025)*100

sum(pds_new_all_DE$tot_rel)
((441177.6-sum(pds_new_all_DE$tot_rel))/441177.6)*100

pds_new_all_DE$tot_sf_cat=pds_new_all_DE$tot_keep+pds_new_all_DE$tot_rel
sum(pds_new_all_DE$tot_sf_cat)
((532202.6-sum(pds_new_all_DE$tot_sf_cat))/532202.6)*100



sum(pds_new_all_DE$tot_keep_bsb)
((14347.9-sum(pds_new_all_DE$tot_keep_bsb))/14347.9)*100

sum(pds_new_all_DE$tot_rel_bsb)
((128101.6-sum(pds_new_all_DE$tot_rel_bsb))/128101.6)*100

pds_new_all_DE$tot_bsb_cat=pds_new_all_DE$tot_keep_bsb+pds_new_all_DE$tot_rel_bsb
sum(pds_new_all_DE$tot_bsb_cat)
((142449.5-sum(pds_new_all_DE$tot_bsb_cat))/142449.5)*100




sum(pds_new_all_DE$tot_keep_wf)
((3184.7222-sum(pds_new_all_DE$tot_keep_wf))/3184.7222)*100

sum(pds_new_all_DE$tot_rel_wf)
((52857.453-sum(pds_new_all_DE$tot_rel_wf))/52857.453)*100

pds_new_all_DE$tot_wf_cat=pds_new_all_DE$tot_keep_wf+pds_new_all_DE$tot_rel_wf
sum(pds_new_all_DE$tot_wf_cat)
((56042.176-sum(pds_new_all_DE$tot_wf_cat))/56042.176)*100
