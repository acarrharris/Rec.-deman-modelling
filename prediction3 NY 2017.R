...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    bsb_sc_data <- subset(costs_new_all_NY, period==p & catch_draw==i, select=c(period, catch_draw, tripid,tot_keep_scup_base, tot_rel_scup_base)) %>%
      tibble()
...
for(p in levels(periodz)){
  ...
  cost_data <- subset(costs_new_all_NY, period == p, select=-c(period, tot_sf_catch))
...
pds_new_all_NY$alt_regs = 1
pds_new_all_NY=subset(pds_new_all_NY, select=-c(Group.1, tot_keep_sf_base, tot_rel_sf_base, 
