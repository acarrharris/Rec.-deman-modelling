...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=0.945, 1,0) 
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.935, 1,0)
    ...
    if(wf_bag>0){
      ...
      wf_catch_data1$keep = ifelse(wf_catch_data1$uniform>=.77, 1,0) 
...
((57421.9-sum(pds_new_all_MD$tot_keep))/57421.9)*100

sum(pds_new_all_MD$tot_rel)
((862081.7-sum(pds_new_all_MD$tot_rel))/862081.7)*100

pds_new_all_MD$tot_sf_cat=pds_new_all_MD$tot_keep+pds_new_all_MD$tot_rel
sum(pds_new_all_MD$tot_sf_cat)
(((57421.9+862081.7)-sum(pds_new_all_MD$tot_sf_cat))/862081.7)*100


sum(pds_new_all_MD$tot_keep_bsb)
((25072.84-sum(pds_new_all_MD$tot_keep_bsb))/25072.84)*100

sum(pds_new_all_MD$tot_rel_bsb)
((158830.3-sum(pds_new_all_MD$tot_rel_bsb))/158830.3)*100

pds_new_all_MD$tot_bsb_cat=pds_new_all_MD$tot_keep_bsb+pds_new_all_MD$tot_rel_bsb
sum(pds_new_all_MD$tot_bsb_cat)
(((25072.84+158830.3)-sum(pds_new_all_MD$tot_sf_cat))/(25072.84+158830.3))*100




sum(pds_new_all_MD$tot_keep_wf)
(( 6751.686-sum(pds_new_all_MD$tot_keep_wf))/ 6751.686)*100

sum(pds_new_all_MD$tot_rel_wf)
((13503.37-sum(pds_new_all_MD$tot_rel_wf))/13503.37)*100

pds_new_all_MD$tot_wf_cat=pds_new_all_MD$tot_keep_wf+pds_new_all_MD$tot_rel_wf
sum(pds_new_all_MD$tot_wf_cat)
(((6751.686+13503.37)-sum(pds_new_all_MD$tot_wf_cat))/(6751.686+13503.37))*100
