...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.77, 1,0) 
    ...
    sc_data=subset(costs_new_all_CT, period ==p & catch_draw==i, select=c(tripid,tot_keep_scup_base, tot_rel_scup_base )) 
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    utility_state0_CT[[p]]=mean_trip_data_s0
...
utility_state0_CT_all=list.stack(utility_state0_CT, fill=TRUE)
