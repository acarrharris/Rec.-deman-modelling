...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    if(fluke_bag>0){
      ...
      sf_catch_data1$keep = ifelse(sf_catch_data1$uniform>=.75, 1,0)
    ...
    if(bsb_bag>0){
      ...
      bsb_catch_data1$keep = ifelse(bsb_catch_data1$uniform>=.05, 1,0) 
    ...
    if(scup_bag>0){
      ...
      scup_catch_data1$keep = ifelse(scup_catch_data1$uniform>=0.62, 1,0) 
...
for(d in 1:100) {
  ...
  param_draws_MA <- param_draws_MA[1:n_drawz, ] 
  
  utilites_MA[[d]]= param_draws_MA
  
}

utilites_MA_all=list.stack(utilites_MA, fill=TRUE)
...
for(p in levels(periodz)){
  ...
  costs_new_MA[[p]] = subset(trip_data, select=c(tripid, cost, catch_draw, tot_keep, tot_rel, tot_sf_catch,
                                                 tot_keep_bsb,tot_rel_bsb,tot_keep_scup, tot_rel_scup  ))
  
  names(costs_new_MA[[p]])[names(costs_new_MA[[p]]) == "tot_keep"] = "tot_keep_sf_base"
  names(costs_new_MA[[p]])[names(costs_new_MA[[p]]) == "tot_rel"] = "tot_rel_sf_base"
  names(costs_new_MA[[p]])[names(costs_new_MA[[p]]) == "tot_keep_bsb"] = "tot_keep_bsb_base"
  names(costs_new_MA[[p]])[names(costs_new_MA[[p]]) == "tot_rel_bsb"] = "tot_rel_bsb_base"
  names(costs_new_MA[[p]])[names(costs_new_MA[[p]]) == "tot_keep_scup"] = "tot_keep_scup_base"
  names(costs_new_MA[[p]])[names(costs_new_MA[[p]]) == "tot_rel_scup"] = "tot_rel_scup_base"
  
  
  costs_new_MA[[p]]$period = p
  ...
  for(d in 1:1) {
    ...
    parameter_draws_MA[[d]]=aggregate_trip_data
    
    
  }
  
  # Combine the output from all the utility parameter draws (for now we only have one)
  parameter_draws_all_MA = as.data.frame(bind_rows(parameter_draws_MA[[1]]))
  parameter_draws_all_MA[is.na(parameter_draws_all_MA)] = 0
  rownames(parameter_draws_all_MA) = NULL
  
  
  parameter_draws_all_MA$period=p
  pds_new[[p]]=parameter_draws_all_MA
...
pds_new_all_MA$alt_regs = 0
pds_new_all_MA= subset(pds_new_all_MA, select=-c(Group.1, tot_sf_catch, tot_bsb_catch))
rm(pds_new)
...
((65366.66-sum(pds_new_all_MA$tot_keep))/65366.66)*100

sum(pds_new_all_MA$tot_rel)
((110457.9-sum(pds_new_all_MA$tot_rel))/110457.9)*100

pds_new_all_MA$tot_sf_cat=pds_new_all_MA$tot_keep+pds_new_all_MA$tot_rel
sum(pds_new_all_MA$tot_sf_cat)
(((65366.66+110457.9)-sum(pds_new_all_MA$tot_sf_cat))/(65366.66+110457.9))*100



sum(pds_new_all_MA$tot_keep_bsb)
((130930.4-sum(pds_new_all_MA$tot_keep_bsb))/130930.4)*100

sum(pds_new_all_MA$tot_rel_bsb)
((266256.6-sum(pds_new_all_MA$tot_rel_bsb))/266256.6)*100

pds_new_all_MA$tot_bsb_cat=pds_new_all_MA$tot_keep_bsb+pds_new_all_MA$tot_rel_bsb
sum(pds_new_all_MA$tot_bsb_cat)
(((130930.4+266256.6)-sum(pds_new_all_MA$tot_bsb_cat))/(130930.4+266256.6))*100



sum(pds_new_all_MA$tot_keep_scup)
((50467.2-sum(pds_new_all_MA$tot_keep_scup))/50467.2)*100

sum(pds_new_all_MA$tot_rel_scup)
((280893.2-sum(pds_new_all_MA$tot_rel_scup))/280893.2)*100

pds_new_all_MA$tot_scup_cat=pds_new_all_MA$tot_keep_scup+pds_new_all_MA$tot_rel_scup
sum(pds_new_all_MA$tot_scup_cat)
(((50467.2+280893.2)-sum(pds_new_all_MA$tot_scup_cat))/(50467.2+280893.2))*100